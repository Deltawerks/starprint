...
class SCManager:
    ...
    def export_item(self, guid: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                [str(CGF_CONVERTER), str(cgf_local_path), "-glb", "-objectdir", str(obj_dir), "-notex"],
                capture_output=True,
                text=True,
        ...
        # Locate the generated GLB file
        glb_file = cgf_local_path.with_suffix('.glb')
        
        # If not found, search recursively in export path
        if not glb_file.exists():
            print(f"GLB not found at expected path {glb_file}, scanning...")
            glbs = list(export_path.rglob("*.glb"))
            if glbs:
                glb_file = glbs[0]
                print(f"Found GLB: {glb_file}")
            else:
                raise Exception("No GLB file generated by converter")
        
        print(f"GLB generated at: {glb_file} (Size: {glb_file.stat().st_size} bytes)")
        
        # Post-Processing: Load GLB directly (no stripping needed)
        try:
            print("Loading GLB mesh...")
            
            # Load GLB with Trimesh
            mesh = trimesh.load(glb_file, force='scene')
    ...
    def export_item_blueprint(self, guid: str) -> Dict[str, Any]:
        ...
        # Helper to convert single file
        def convert_to_glb(rel_path):
            ...
            glb_file = src_file.with_suffix(".glb")
            if glb_file.exists():
                return glb_file
            
            # Convert
            cmd = [str(CGF_CONVERTER), str(src_file), "-glb", "-objectdir", str(export_path), "-notex"]
            try:
        ...
        for geom_key, geom_data in bp.geometry.items():
            ...
            # Convert to GLB
            glb_path = convert_to_glb(geom_key)
            if not glb_path:
